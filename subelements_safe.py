...
#
# THIS MODULE IS A COPY OF lib/ansible/runner/lookup_plugins/subelements.py
# FROM ANSIBLE SOURCE TREE THAT DOES NOT FAIL IF LOOKUP IS NOT FOUND.
#
#
    ...
    def run(self, terms, inject=None, **kwargs):
        ...
        for item0 in elementlist:
            if not isinstance(item0, dict):
                # If variable couldn't have be resolved, silently skip,
                # display a warning message.
                if term0 == item0:
                    utils.warning(
                        'Could not resolve variable "%s" is it defined for %s?'
                        % (item0, inject['inventory_hostname']))
                    continue
            ...
            if not subelement in item0:
                ...
                #
                # This prevent from getting an error if subelement key is
                # not defined.
                continue
